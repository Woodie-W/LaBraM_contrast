...
def train_one_epoch(model: torch.nn.Module,
    ...
    for i,(data_loader, ch_names) in enumerate(zip(data_loader_list, ch_names_list)):
        print("Now training: ",datasets_train[i])
        if len(data_loader) == 0:
            print(f"DataLoader for {datasets_train[i]} is empty (sample < batchsize), skipping.")
            continue
        input_chans = utils.get_input_chans(ch_names)
        for step, (batch,filename) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            if not math.isfinite(loss_value):
                error_message = f"loss_value: {loss_value}, dataset: {datasets_train[i]}, step: {step}, file: {filename}"
                # print(error_message)
            ...
            if not math.isfinite(grad_norm):
                error_message = f"grad_norm: {grad_norm}, dataset: {datasets_train[i]}, step: {step}, file: {filename}"
                # print(error_message)
                with open(error_file_path, "a") as error_file:
                    error_file.write(error_message + "\n")
...
@torch.no_grad()
def evaluate(data_loader_list, model, device, log_writer=None, epoch=None, ch_names_list=None, args=None,datasets_val=None):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Validation:'
    ...
    for i,(data_loader, ch_names) in enumerate(zip(data_loader_list, ch_names_list)):
        print("Now testing: ",datasets_val[i])
        input_chans = utils.get_input_chans(ch_names)
        for step, (batch,filename) in enumerate(metric_logger.log_every(data_loader, 10, header)):
            images = batch.float().to(device, non_blocking=True) / 100
            loss, log_loss = model(images, input_chans=input_chans)
