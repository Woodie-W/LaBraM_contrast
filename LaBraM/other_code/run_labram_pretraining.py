...
def get_args():
    ...
    parser.add_argument('--weight_decay', type=float, default=0.05,
                        help='weight decay (default: 0.05),set 1e-4')
    parser.add_argument('--weight_decay_end', type=float, default=None, help="""Final value of the
        weight decay. We use a cosine schedule for WD. 
        (Set the same value with args.weight_decay to keep weight decay no change)""")

    parser.add_argument('--lr', type=float, default=5e-4, metavar='LR',
                        help='learning rate (default: 5e-4),set 1e-6')
...
def main(args):
    ...
    datasets_train = [
            ...
            ['./pretrain_set/EEG/SXMU_1_PROCESSED/MDD/72chs', ], # loss up
            ['./pretrain_set/EEG/SXMU_1_PROCESSED/HC/72chs', ], # loss up
            ['./pretrain_set/EEG/TUHEEG_PROCESSED/tuh_eeg_abnormal/train/abnormal/19chs', ], # loss up
    ...
    dataset_train_list, train_ch_names_list = utils.build_pretraining_dataset(datasets_train, time_window,
                                                                              stride_size=200)
    # 记录一下数据集，方便记录loss
    args.datasets_train = datasets_train
