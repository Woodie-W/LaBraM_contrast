...
class Subject_data():
    ...
    class EEG_data:
        ...
        def save_data(self, save_folder, total_id):
            ...
            # 保存数据切片文件
            # 对应每份数据取多少的时间长度
            # second_dict = {60: 4, 61: 4, 129: 2, 30: 8, 59: 4, 72: 4, 19: 13, 20: 13, 17: 15, 26: 10, 65: 4}
            # len_windows = second_dict[n_chns] * 200
            seconds = round(256 / n_chns)
            len_windows = seconds * 200
            # 步长 len_windows * 0.8
            slices = []
...
def process_subject(file_path, save_folder):
    ...
    try: num_cores = len(psutil.Process().cpu_affinity())
    except AttributeError:
        # 某些系统可能没有 cpu_affinity()，回退到 cpu_count()
        num_cores = psutil.cpu_count(logical=False)

    set_affinity(total_id, num_cores)
...
if __name__ == '__main__':
    ...
    data_folders = [
            '/data/sxk/Tdbrain_challenge_mdd_hc_predict/66_Td_mdd_predict_eyeopen',
            '/data/sxk/Tdbrain_challenge_mdd_hc_predict/65_Td_mdd_predict_eyeclose'
            # '/home/wangkuiyu/data1/LEM/new_data_pool/EEG/LEMON',
            # '/home/wangkuiyu/data1/LEM/new_data_pool/EEG/HBN_1',
    ]
    save_folders = [
            '/data/sxk/pretrain_set/EEG/eyeopen',
            '/data/sxk/pretrain_set/EEG/eyeopen',
    ...
    # ]
    #
    # save_folder = '/data/sxk/pretrain_set/EEG/all'
    # file_paths = []
    #
