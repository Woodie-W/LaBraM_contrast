...
from engine_for_finetuning import (
    train_one_epoch,
    evaluate,
    train_one_epoch_for_parkinson,
    evaluate_for_parkinson,
)
...
def get_dataset(args):
    ...
    dataset_train = [
        [path]
        for name, paths in all_datasets.items()
        if name != args.dataset
        for path in paths
    ]
    args.dataset_train = dataset_train
    # print("Dataset_train = %s" % str(dataset_train))
    dataset_val = all_datasets.get(args.dataset, [])
    dataset_val = [[dataset] for dataset in dataset_val]

    print("dataset_train", dataset_train)
    print("dataset_val", dataset_val)

    dataset_train_list, train_ch_names_list = utils.build_finetuning_dataset(
        dataset_train,
        label2int={"HC": 0, "HEALTHY": 0, "LowOCD": 0, "PD": 1, "PARKINSON": 1},
    )

    dataset_val_list, val_ch_names_list = utils.build_finetuning_dataset(
        dataset_val,
        label2int={"HC": 0, "HEALTHY": 0, "LowOCD": 0, "PD": 1, "PARKINSON": 1},
    )
    ...
    # print(train_ch_names_list,val_ch_names_list)
...
def main(args, ds_init):
    ...
    num_training_steps_per_epoch = (
        sum([len(dataset) for dataset in dataset_train_list]) // total_batch_size
    )
    print("LR = %.8f" % args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_for_parkinson(
            model,
            criterion,
            data_loader_train_list,
            optimizer,
        ...
        if data_loader_val_list is not None:
            val_stats = evaluate_for_parkinson(
                data_loader_val_list,
                model,
                device,
                header="Val:",
