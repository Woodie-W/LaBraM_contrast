...
def train_one_epoch(model: torch.nn.Module, vqkd: torch.nn.Module,
    ...
    for i,(data_loader, ch_names) in enumerate(zip(data_loader_list, ch_names_list)):
        if len(data_loader) == 0:
            print(f"DataLoader is empty (sample < batchsize), skipping.")
            continue
        dataset_name = args.datasets_train[i]
        ...
        for step, (batch) in enumerate(
            ...
            metric_logger.update(loss_rec_sym=loss_rec_sym.item() / 2)
            ...
            if log_writer is not None:
                ...
                log_writer.update(loss_rec_sym=loss_rec_sym.item()/2, head="loss")

            metric_logger.update(loss=loss_value / 2)
            metric_logger.update(loss_scale=loss_scale_value)
            # 记录数据集loss
            # metric_logger.update(f"{dataset_name}_loss"=loss_value / 2)
            metric_logger.update(**{f"{dataset_name}_loss": loss_value / 2})
            ...
            if log_writer is not None:
                log_writer.update(loss=loss_value/2 , head="loss")
                log_writer.update(loss_scale=loss_scale_value, head="opt")
                
                # log_writer.update(f"{dataset_name}_loss"=loss_value / 2, head="loss")
                log_writer.update(**{f"{dataset_name}_loss": loss_value / 2})
