    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            test_stats = evaluate(data_loader_test, model, device, header='Test:', ch_names=ch_names, metrics=metrics,
                                  is_binary=args.nb_classes == 1)
            print(f"Accuracy of the network on the {len(dataset_val)} test.py EEG: {val_stats['accuracy']:.2f}%")
            ...
            print(f'Max accuracy val: {max_accuracy:.2f}%, max accuracy test.py: {max_accuracy_test:.2f}%')
            if log_writer is not None:
                ...
                for key, value in test_stats.items():
                    if key == 'accuracy':
                        log_writer.update(accuracy=value, head="test.py", step=epoch)
                    elif key == 'balanced_accuracy':
                        log_writer.update(balanced_accuracy=value, head="test.py", step=epoch)
                    elif key == 'f1_weighted':
                        log_writer.update(f1_weighted=value, head="test.py", step=epoch)
                    elif key == 'pr_auc':
                        log_writer.update(pr_auc=value, head="test.py", step=epoch)
                    elif key == 'roc_auc':
                        log_writer.update(roc_auc=value, head="test.py", step=epoch)
                    elif key == 'cohen_kappa':
                        log_writer.update(cohen_kappa=value, head="test.py", step=epoch)
                    elif key == 'loss':
                        log_writer.update(loss=value, head="test.py", step=epoch)
