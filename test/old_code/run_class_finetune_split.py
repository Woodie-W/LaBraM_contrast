...
def get_args():
    ...
    parser.add_argument("--split", default=0, type=int , help="split dataset")  # 使用部分数据集训练, td优先
...
def prepare_data(seed, split=True, td=False, data_split=0, json_folder=None):
    seed = 12345
    np.random.seed(seed)
    # 读取数据路径
    ...
    if td: data_path_with_cls = {}
    else: data_path_with_cls = {k: v for d in [hc, mdd] for k, v in d.items()}
    ...
    if td: sft_paths = [
            ...
            # '/data/sxk/labram2_resting_fine_pool/clinical/HUILONGGUAN_1028/30chs',
        ]
    elif data_split == 1: sft_paths = [
            "/data/sxk/labram2_resting_fine_pool/resting_eye_open/Parkinson_eyes_open_PROCESSED/61chs",
            "/data/sxk/labram2_resting_fine_pool/resting_eye_open/Parkinson_eyes_open_PROCESSED/63chs",
            "/data/sxk/labram2_resting_fine_pool/resting_eye_close/AD_FD_HC_PROCESSED/19chs",
            "/data/sxk/labram2_resting_fine_pool/resting_unknown/PREDICT-PD_LPC_Rest/61chs",
    ...
    elif data_split == 4: sft_paths = [
        '/data/sxk/labram2_resting_fine_pool/resting_eye_open/HUAWEI_eye_open/30chs',
        '/data/sxk/labram2_resting_fine_pool/resting_eye_open/HUAWEI_eye_open/59chs',
        '/data/sxk/labram2_resting_fine_pool/resting_eye_close/HUAWEI_eye_close/30chs',
        '/data/sxk/labram2_resting_fine_pool/resting_eye_close/HUAWEI_eye_close/59chs',
        ]
    elif data_split == 5: sft_paths = [
        "/data/sxk/labram2_resting_fine_pool/resting_unknown/LEMON/61chs",
        '/data/sxk/labram2_resting_fine_pool/resting_unknown/57_Predict-Depression-Rest-New/60chs',
        '/data/sxk/labram2_resting_fine_pool/resting_eye_close/60_ANDing/20chs',
        '/data/sxk/labram2_resting_fine_pool/resting_eye_open/60_ANDing/20chs',
    ...
    # 结构如下:{数据集路径 : [fold_0, fold_1, ..., fold_9]}, 每一个list为一个numpy数组, 存放具体数据文件路径
    ...
    for fold_idx in range(10):
        ...
        actual_subjects = distributed_subjects_by_fold_wo_cls[fold_idx] \
                          | distributed_subjects_by_fold_with_cls[fold_idx]  # 取并集
...
def get_dataset(args):
    ...
    # todo 1
    #
    if args.td: kfold_indices_wo_cls, kfold_indices_with_cls = prepare_data(
                seed, data_split= args.split, td = True, json_folder='10fold_split_dict/tdbrain')
    else: kfold_indices_wo_cls, kfold_indices_with_cls = prepare_data(
                seed, data_split= args.split, td = False, json_folder='10fold_split_dict/all')
    ...
    for key, folds in kfold_indices_with_cls.items():  # key = 'HC' or 'MDD', value = [fold_0, fold_1, ..., fold_9]
        ...
        eval_indices_with_cls[key] = eval_fold  # key = 'HC' or 'MDD', value = [pth of .pkl]

    # todo 2
    if (args.nb_classes == 1):
        used_ints = [0, 1]
    else:
        used_ints = [i for i in range(args.nb_classes)]
    ...
    # args.nb_classes = 45  #args传入的是对象的引用,所以可以改变
    # args.nb_classes = 1   #二分类
...
def main(args, ds_init):
    ...
    if ds_init is not None:  # DeepSpeed初始化函数
    ...
    if args.disable_eval_during_finetuning:  # 微调时禁用评估
        dataset_val = None
    ...
    if args.nb_classes == 1:
        ...
        criterion = torch.nn.CrossEntropyLoss()  # 多分类交叉熵损失函数 Cross Entropy Loss
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_for_embedding(
            ...
            args=args,
            used_ints=used_ints,
        )  # 进行一轮训练的所有细节(遍历数据、前向传播、计算损失、反向传播、更新权重等)
            ...
            if log_writer is not None:
                for key, value in val_stats.items():
                    if key == "accuracy":
                        ...
                        log_writer.update(balanced_accuracy_old=value, head="val", step=epoch)
                    elif key == "accuracy_old":
                        log_writer.update(accuracy_old=value, head="val", step=epoch)
