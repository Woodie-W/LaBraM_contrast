...
tdbrain = [ "/data/sxk/labram2_resting_fine_pool/resting_eye_open/Td_eyeopen/26chs",
...
def get_dataset(args):
    ...
    for dataset_key, folds in kfold_indices_wo_cls.items():  # key = dataset_path, value = [fold_0, ..., fold_9]
        train_fold = []
        eval_fold = []
        if dataset_key in tdbrain:  # TDBrain数据集
            for i in range(10):
                if i == args.fold: eval_fold = folds[i].tolist()
                else: train_fold.extend(folds[i].tolist())
        else: 
