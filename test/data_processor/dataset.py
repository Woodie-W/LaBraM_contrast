...
class SFTSet_embedding(Dataset):
    def __init__(
        ...
        print(
            "Distribution of cls labels", label2counts,
            "num samples", len(self.file_paths),
        )

        # todo: 需要检查，后续可能增加
        if label2int is None: self.label2int = default_label2int
        else: self.label2int = label2int
...
def custom_collate_fn(batch, multi_label=False):
    ...
    features = []
    if multi_label:
        all_labels = np.ones((num_samples, 5), dtype=int) * (-1)  # 假设每个被试最多有5个标签，不足用-1填充
    else:
