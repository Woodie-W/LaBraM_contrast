...
def prepare_data(seed, split=True, td=False, json_folder=None):
    seed = 12345
    np.random.seed(seed)
    # 读取数据路径
...
def get_dataset(args):
    ...
    if args.td: kfold_indices_wo_cls, kfold_indices_with_cls = prepare_data(
                seed, td = True, json_folder='10fold_split_dict/tdbrain')
    else: kfold_indices_wo_cls, kfold_indices_with_cls = prepare_data(
                seed, td = False, json_folder='10fold_split_dict/all')
