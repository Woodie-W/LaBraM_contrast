...
# 共享位置编码表
def RelativePositionBias(window_size, num_heads):
    pass
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_norm=None, qk_scale=None, attn_drop=0.,
        ...
        # 窗口自注意力机制，对每一Embedding的每一head进行位置编码
        if window_size:
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        # shape(3,B,self.num_heads,N,C//self.num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_norm=None, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, qk_scale=qk_scale,
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, **kwargs):
        # Batch_size,number of electrodes, number of patches, patch size
        B, C, H, W = x.shape
...
class TemporalConv(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.gelu3(self.norm3(self.conv3(x)))  # B, out_chans, NA, T
        x = rearrange(x, 'B C NA T -> B NA (T C)')  # B, NA, T, out_chans
        return x


class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200,
                 depth=12, num_heads=10, mlp_ratio=4., qkv_bias=False, qk_norm=None, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None, use_abs_pos_emb=True,
                 use_rel_pos_bias=False, use_shared_rel_pos_bias=False, use_mean_pooling=True, init_scale=0.001,
                 num_drugs=5, drug_embed_dim=8,
                 **kwargs):
        super().__init__()
        ...
        self.patch_embed = TemporalConv(out_chans=out_chans) if in_chans == 1 \
        ...
        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, 300 + 1, embed_dim), requires_grad=True)
            # self.pos_embed = nn.Parameter(torch.zeros(1, 129, embed_dim), requires_grad=True)
        else:
            self.pos_embed = None
        ...
        ## --- 新增药物嵌入层 --- 
        self.drug_embedding = nn.Embedding(num_embeddings=num_drugs, embedding_dim=drug_embed_dim)
        # self.head = nn.Linear(embed_dim + drug_embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        head_input_dim = embed_dim + drug_embed_dim + 1
        self.head = nn.Linear(head_input_dim, 1) if num_classes > 0 else nn.Identity()
        
        # 默认就是200的embed_dim
        self.embedding_head = nn.Linear(embed_dim, embed_dim)
        ...
        if isinstance(self.head, nn.Linear):
            ...
            #初始化为较小值
            print("\n" + "="*50)
            print("INFO: Applying custom initialization for the regression head.")
            # 用一个非常小的标准差重新初始化权重
            torch.nn.init.normal_(self.head.weight, mean=0.0, std=0.01)
            # 将偏置初始化为0
            if self.head.bias is not None:
                torch.nn.init.constant_(self.head.bias, 0)
            print("  - Head weight initialized with N(0, 0.01)")
            print("  - Head bias initialized to 0")
            print("="*50 + "\n")    
    ...
    # return_patch_tokens
    # def forward(self, x, drug_ids, input_chans=None, return_patch_tokens=True, return_all_tokens=False, **kwargs):
        '''
        ...
        '''
        ## 原来代码中的向前传递项拼接药物嵌入
        x_eeg = self.forward_features(x, input_chans=input_chans, return_patch_tokens=return_patch_tokens,
                                  return_all_tokens=return_all_tokens, **kwargs)
        # 可以在这里保存一下embedding,需要传入参数subject_id
        x_eeg = x_eeg.sum(1)

        ## 获取药物嵌入
        x_drug = self.drug_embedding(drug_ids)
        fused_embedding = torch.cat([x_eeg, x_drug], dim=-1)
        output = self.head(fused_embedding)
        
        return output

    #增加了hamd拼接
    def forward(self, x, drug_ids, hamd0_normalized, input_chans=None, **kwargs):
        """
        接收标准化的HAMD₀分数作为直接输入。
        x: [B, N, A, T] EEG 特征
        drug_ids: [B] 药物ID
        hamd0_normalized: [B] 经过标准化处理的HAMD₀分数
        """
        # 1. 提取EEG特征并进行平均池化
        x_eeg = self.forward_features(x, input_chans=input_chans, return_patch_tokens=True)
        x_eeg = x_eeg.mean(1) # [B, embed_dim]
        # 2. 获取药物嵌入
        x_drug = self.drug_embedding(drug_ids) # [B, drug_embed_dim]
        # 3. 增加HAMD₀的维度以便拼接
        # hamd0_normalized 的输入形状是 [B], .unsqueeze(-1) 将其变为 [B, 1]
        hamd0_feature = hamd0_normalized.unsqueeze(-1)
        # 4. 特征拼接 (Feature Concatenation)
        fused_embedding = torch.cat([x_eeg, x_drug, hamd0_feature], dim=-1)
        # 5. 通过回归头得到预测值
        output = self.head(fused_embedding).squeeze(-1) # squeeze(-1) 将 [B, 1] 变为 [B]    
        return output
...
# 定义了三种不同大小的LaBraM， ## 增加num_drugs 和 drug_embed_dim 参数
@register_model
