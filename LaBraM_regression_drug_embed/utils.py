...
def all_gather_batch(tensors):
    ...
    if world_size == 1: return tensors
    tensor_list = []
    output_tensor = []
...
def get_multi_label(label):
    ...
    if label is None:
        ...
        for item in label: items.append(item)
    else:
...
def get_metrics(output, target, metrics, is_binary, threshold=0.5):
    if is_binary:   # to prevent all 0 or all 1 and raise the AUROC error
    ...
    else: results = multiclass_metrics_fn(target, output, metrics=metrics)
    return results
